...
function Paddle:update(dt, mouseX, mouseY)
    -- update paddle width respect to its size. Its heigth is always constant
    ...
    -- keyboard input
    if love.keyboard.isDown('left') or ((not MouseIn(Button(Buttons['pause']), x, y)) and (love.mouse.isDown(1) and x < VIRTUAL_WIDTH / 2)) then
        self.dx = -PADDLE_SPEED
    elseif love.keyboard.isDown('right') or (love.mouse.isDown(1) and x > VIRTUAL_WIDTH / 2) then
        self.dx = PADDLE_SPEED
    else
        self.dx = 0
    end
